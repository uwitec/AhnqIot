...
namespace SmartIot.Tool.Core.Device.Transport
{
    public class TransportFactory
    {
        private static readonly ConcurrentDictionary<string, IPAddress> HostCache;
        ...
        static TransportFactory()
        {
            ...
            HostCache = new ConcurrentDictionary<string, IPAddress>();
        }
        ...
        private static ITransport CreateNetTransport(TransportTypeEnum type, string host, int e, int timeout)
        {
            ...
            var session = new NetUri("{type.ToString()}://{ip}:{e}").CreateRemote();
            session.Log = XTrace.Log;
            session.Error += Session_Error;
            //if (session.GetType() == typeof(TcpSession))
            //{
        ...
        {
            var key = "{0}://{1}:{2}".F(type.ToString(), host, e);
            ...
            lock (TransportsCache)
            {
                if (TransportsCache.ContainsKey(key))
                {
                    ...
                    else
                    {
                        if (type != TransportTypeEnum.Rs232)
                        {
                            if (type != TransportTypeEnum.Tcp && type != TransportTypeEnum.Udp) return trans;
                            var session = trans as SessionBase;
                            if (session != null && !session.Disposed && session.Active) return trans;
                            TransportsCache.Remove(key);
                        ...
                        else
                        {
                            ...
                            if (sp != null && !sp.Disposed) return trans;
                            TransportsCache.Remove(key);
