...
namespace AhnqIot.Dal
{
    ...
    public partial class Log : Entity<Log>
    {
        ...
        //    if (XTrace.Debug) XTrace.WriteLine("开始初始化{0}[{1}]数据……", typeof(Log).Name, Meta.Table.DataTable.DisplayName);
        ...
        //    if (XTrace.Debug) XTrace.WriteLine("完成初始化{0}[{1}]数据！", typeof(Log).Name, Meta.Table.DataTable.DisplayName);
        //}
        ...
        public static EntityList<Log> FindAllByCategory(String category)
        {
            if (Meta.Count >= 1000)
                return FindAll(__.Category, category);
            else // 实体缓存
        ...
        /// <returns>实体集</returns>
        public static EntityList<Log> Search(Int32 userid, DateTime start, DateTime end, String key, PageParameter param)
        {
