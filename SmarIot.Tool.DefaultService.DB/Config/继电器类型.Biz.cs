...
namespace SmartIot.Tool.DefaultService.DB
{
    ...
    public class RelayType : RelayType<RelayType>
    {
    }

    /// <summary>控制工作类型</summary>
    public partial class RelayType<TEntity> : LogEntity<TEntity> where TEntity : RelayType<TEntity>, new()
    {
        #region 高级查询
        ...
        protected override void InitData()
        {
            ...
            AddEntity("Hold_Two", "保持继电器两态");
            AddEntity("Hold_Three", "保持继电器三态");
            AddEntity("Pluse_Two", "脉冲继电器两态");
            AddEntity("Pluse_Three", "脉冲继电器三态");
            if (XTrace.Debug)
        ...
        public EntityList<ControlDeviceUnit> ControlDeviceUnits
        {
            get
            {
                if (_ControlDeviceUnits == null && !String.IsNullOrEmpty(Id.ToString()) &&
                    !Dirtys.ContainsKey("ControlDeviceUnits"))
                {
                    _ControlDeviceUnits = ControlDeviceUnit.FindAllByRelayTypeId(Id);
