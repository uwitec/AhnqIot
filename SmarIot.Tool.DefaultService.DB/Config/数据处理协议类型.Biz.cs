...
namespace SmartIot.Tool.DefaultService.DB
{
    ...
    public class ProtocalType : ProtocalType<ProtocalType>
    {
    }

    /// <summary>数据处理协议类型</summary>
    public partial class ProtocalType<TEntity> : LogEntity<TEntity> where TEntity : ProtocalType<TEntity>, new()
    {
        #region 高级查询
        ...
        /// <summary>该数据处理协议类型所拥有的模块化设备集合</summary>
        [XmlIgnore]
        public EntityList<ModularDevice> ModularDevices
        {
            get
            {
                ...
                {
                    _ModularDevices = ModularDevice.FindAllByProtocalTypeID(ID);
