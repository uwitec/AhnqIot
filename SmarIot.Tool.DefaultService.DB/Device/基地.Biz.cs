...
namespace SmartIot.Tool.DefaultService.DB
{
    ...
    public class Farm : Farm<Farm>
    {
    }

    /// <summary>基地</summary>
    public partial class Farm<TEntity> : LogEntity<TEntity> where TEntity : Farm<TEntity>, new()
    {
        #region 高级查询
        ...
        /// <summary>该基地所拥有的基地设施集合</summary>
        [XmlIgnore]
        public EntityList<Facility> Facilitys
        {
            get
            {
                if (_Facilitys == null && ID > 0 && !Dirtys.ContainsKey("Facilitys"))
                {
                    _Facilitys = Facility.FindAllByFarmID(ID);
        ...
        public static new EntityList<TEntity> FindAll()
        {
            return FindAllWithCache();
            //else // 实体缓存
            //return Meta.Cache.Entities.FindAll()
            // 单对象缓存
        ...
        /// <returns></returns>
        [DataObjectMethod(DataObjectMethodType.Select, false)]
        public static TEntity FindByCode1(string serialnum)
        {
            if (Meta.Count >= 1000)
                return Find(__.Code1, serialnum);
            else // 实体缓存
                return Meta.Cache.Entities.Find(__.Code1, serialnum);
            // 单对象缓存
            //return Meta.SingleCache[id];
        }
