...
namespace SmartIot.Tool.DefaultService.DB
{
    ...
    public class ApiAccessLog : ApiAccessLog<ApiAccessLog>
    {
    }

    /// <summary>接口通讯记录</summary>
    public partial class ApiAccessLog<TEntity> : LogEntity<TEntity> where TEntity : ApiAccessLog<TEntity>, new()
    {
        #region 高级查询
        ...
        /// <summary>根据接口名称查找</summary>
        /// <param name="apiname">接口名称</param>
        /// <returns></returns>
        [DataObjectMethod(DataObjectMethodType.Select, false)]
        public static EntityList<TEntity> FindAllByApiName(String apiname)
        {
            if (Meta.Count >= 1000)
                return FindAll(__.ApiName, apiname);
            else // 实体缓存
                return Meta.Cache.Entities.FindAll(__.ApiName, apiname);
        }
